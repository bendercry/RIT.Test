...
* Первый раз с sqlite работал и, во-первых, не совсем понял когда конкретно мне надо вызывать insert в бд и т.к. физического устройства у меня нет прочекать как адекватно это работает тоже не смог. Добавил insert в reset/(10 min expired) и в func applicationDidEnterBackground
